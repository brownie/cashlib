...
class CLSignatureVerifier {
	public:
		CLSignatureVerifier(Ptr<const GroupRSA> publicKey, 
							Ptr<const Group> comGroup, int lx, const vector<ZZ> &c,
							int numPrivates, int numPublics);

		CLSignatureVerifier(Ptr<const GroupRSA> pk, int lx, int numPrivates,
