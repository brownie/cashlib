...
void setupEnvironment(Environment &e, const string& fname) {
	...
	if (fname.find("ecash.txt") != string::npos) {
		...
		Ptr<GroupPrime> cashG = new GroupPrime("bank", 1024, stat*2, stat); // init gen is f
		...
		Ptr<GroupRSA> rangeG = new GroupRSA("first", 1024, stat); // first gen is g1
		...
		Ptr<GroupPrime> cashG = new GroupPrime("bank", 1024, 2*stat, stat); // this is g
		cashG->addNewGenerator(); // this is h
		e.groups["G"] = cashG;	
		...
		Ptr<GroupPrime> cashG = new GroupPrime("bank", 1024, stat*2, stat);
		cashG->addNewGenerator(); // this is h
		e.groups["G"] = cashG;	
		...
		Ptr<GroupRSA> cashG = new GroupRSA("bank", 1024, stat); // this is g
		//Ptr<GroupPrime> cashG = new GroupPrime("bank", 1024, stat*2, stat);
		cashG->addNewGenerator(); // this is h
		e.groups["G"] = cashG;	
		e.variables["J"] = to_ZZ(51);
